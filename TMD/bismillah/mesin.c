...
//PROSEDUR UNTUK MENGHAPUS CHILD
    ...
    if(root != NULL)                                                    //jika tree tidak kosong
    {
        ...
        if(root->child != NULL)
        {
            if(root->child->sibling == NULL)
            {
                ...
                root->child = NULL;                                     //root child menjadi null
            }
...
//PROSEDUR UNTUK MENGHAPUS KESELURUHAN CHILD
...
{
    int tanda = 0;                                                        //inisialisasi penanda
    int j = 0, k = 0, l = 0, m = 0;                                       //inisialisasi penanda

    for (int i = 0; i < strlen(str); i++)                                 //perulangan sebanyak panjang string inputan
    {
        if(str[i] == '#')                                                 //jika bertemu pagar
        {
            tanda++;                                                      //iterasi tanda
        }
        else if(tanda == 0)                                               //jika tanda masih nol, artinya nama childnya
        {
            temp1[j] = str[i];                                            //masukkan string sebelum pagar ke temp1
            j++;                                                          //iterasi
        }
        else if(tanda == 1)                                               //jika tandanya 1, artinya nama parentnya
        {
            temp2[k] = str[i];                                            //masukkan string yg diapit dua pagar ke temp2
            k++;                                                          //iterasi
        }
        else if(tanda == 2)                                               //jika tandanya 2, artinya nilai jualnya
        {
            temp3[l] = str[i];                                            //masukkan string yg diapit dua pagar ke temp3
            l++;                                                          //iterasi
        }
        else if(tanda == 3) //j                                           //jika tandanya 3, artinya jumlah komponennya
        {
            temp4[m] = str[i];                                            //masukkan string terakhir ke temp 4
            m++;                                                          //iterasi
        }
    }
    
    //TUTUP STRING
    temp1[j] = '\0';                                                     //tutup string nama child
    temp2[k] = '\0';                                                     //tutup string parent
    temp3[l] = '\0';                                                     //tutup string nilai jual
    temp4[m] = '\0';                                                     //tutup string jumlah komponen
    *jumlah = temp4[0] - 48;                                             //ubah string jumlah komponen ke integer
}
...
void cariMaxLen(simpul *root, int level, int temp[])
{
    if (root != NULL)                                                                                                   //jika tree tidak kosong
    {
       if(level == root->kontainer.level)                                                                               //jika level saat ini adalah level pada struck
       {
            if(temp[root->kontainer.level] < strlen(root->kontainer.nama) + 4 + strlen(root->kontainer.nilaiJual))      //jika temp lebih pendek dari string saat ini
            {
                temp[root->kontainer.level] = strlen(root->kontainer.nama) + 4 + strlen(root->kontainer.nilaiJual);     //ganti temp dengan string saat ini
            }

            for (int i = 0; i < root->kontainer.simpan; i++)                                                            //jika lebih panjang, lanjut cek komponen
            {
                if(temp[root->kontainer.level] < strlen(root->kontainer.komponen[i]) + 4)                               //jika temp lebih pendek dari string komponen saat ini
                {
                    temp[root->kontainer.level] = strlen(root->kontainer.komponen[i]) + 4;                              //ganti temp dengan string saat ini
                }   
            }
       }

        simpul *bantu = root->child;                                                    //buat pointer bantu
        if(bantu != NULL)                                                               //jika bantu tidak null
        {
            ...
            if(bantu->sibling == NULL)                            
            {
                cariMaxLen(bantu, level, temp);                                         //rekursif untuk mencari string terpanjang
            }
            
            //JIKA LEBIH DARI SATU ANAK
            else
            {
                while(bantu->sibling != root->child)                                   //selama bantu siblingnya bukan root child
                {
                   cariMaxLen(bantu, level, temp);                                     //rekursif untuk mencari string terpanjang
                   bantu = bantu->sibling;                                             //bantu maju ke siblingnya
                }
            }
            cariMaxLen(bantu, level, temp);                                            //rekursif untuk mencari string terpanjang
        }
...
//PROSEDUR UNTUK MENGHAPUS CHILD YANG KURANG DARI BATAS JUAL
void hapus_jual(simpul *root, int batasJual)
{ 
    if(root != NULL)                                                            //jika tree tidak kosong
    {  
        simpul *bantu = root->child;                                            //buat pointer bantu yang menunjuk pada root child
        if(bantu != NULL)                                                       //jika bantu bukan null                                  
        {
            //JIKA HANYA MEMILIKI SATU ANAK
            if(bantu->sibling == NULL)
            {
                if(bantu->kontainer.nilaiJual2 < batasJual)                     //jika nilai jualnya lebih kecil dari batas jual
                {
                    delChild(bantu->kontainer, root);                           //hapus child
                }
                else                                                            //jika lebih besar
                {
                    hapus_jual(bantu, batasJual);                               //rekursif
                }
            }
            ...
            {
                while(bantu->sibling != root->child && bantu->sibling != NULL)      //selama sibling anak yang ditunjuk pointer bantu bukan merupakan anak yg berhubungan langsung dengan root
                {
                    simpul *sibling = bantu->sibling;                               //buat pointer sibling
                    if(bantu->kontainer.nilaiJual2 < batasJual)                     //jika nilai jualnya lebih kecil dari batas jual
                    {
                        delChild(bantu->kontainer, root);                           //hapus child
                    }
